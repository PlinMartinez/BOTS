...
while True:
    ...
    for par in pares:
        ...
        def ordenar_df1(par):
            ...
            del [df['timestamp'], df['datetime'],
                 df['side'], df['amount'], df['cost']]
        ...
        libro_ordenes = binance.fetch_order_book(par, 100)
        ...
        minimo_notional = float(
            mercados[par]['info']['filters'][3]['minNotional'])
        precision = mercados[par]['precision']
        ...
        if media > antes and media > despues:
            ...
            try:
                orden = binance.create_order(symbol, type, side, amount, price)
                print('hecha orden de venta ' + symbol + ' '+ str(price))
            except:
                print ('No se ha podido hacer la orden de venta, supongo que fallo saldo')
        ...
        if media < antes and media < despues:
            ...
            try:
                orden = binance.create_order(symbol, type, side, amount, price)
                print('hecha orden de compra ' + symbol + str(price))
            except:
                print('No se ha podido hacer la orden de compra, supongo que fallo saldo')
